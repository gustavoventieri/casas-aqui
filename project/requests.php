...
if(isset($_POST['delete'])){
   ...
   if($verify_delete->rowCount() > 0){
      ...
      $success_msg[] = 'Requisição excluida com sucesso!';
   }else{
      $warning_msg[] = 'Requisição já excluida!';
   }
...
<head>
   ...
   <title>Propostas - Casas Aqui</title>
...
<section class="requests">
   ...
   <h1 class="heading">Todas propostas recebidas</h1>
   ...
   <div class="box">
      ...
      <p>Listagem: <span><?= $fetch_property['property_name']; ?></span></p>
      <form action="" method="POST">
         <input type="hidden" name="request_id" value="<?= $fetch_request['id']; ?>">
         <input type="submit" value="Recusar proposta" class="btn" onclick="return confirm('essa proposta essa proposta?');" name="delete">
         <a href="view_property.php?get_id=<?= $fetch_property['id']; ?>" class="btn">Ver propriedades </a>
      </form>
   </div>
   <?php
    }
   }else{
      echo '<p class="empty">Você não tem nenhuma requisição!</p>';
   }
   ?>
