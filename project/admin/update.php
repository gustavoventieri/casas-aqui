...
if(isset($_POST['submit'])){
   ...
   if(!empty($name)){
      ...
      if($verify_name->rowCount() > 0){
         $warning_msg[] = 'Username já existente!';
      }else{
         $update_name = $conn->prepare("UPDATE `admins` SET name = ? WHERE id = ?");
         $update_name->execute([$name, $admin_id]);
         $success_msg[] = 'Username atualizado';
      }
   }
...
<head>
   ...
   <title>Atualizar Conta - Casas Aqui</title>
...
<section class="form-container">
   ...
   <form action="" method="POST">
      ...
      <input type="password" name="c_pass" placeholder="Confirme a nova senha" maxlength="20" class="box" oninput="this.value = this.value.replace(/\s/g, '')">
